...
class WebFont {
	...
	public function __construct($url = '', $styles_dir = 'styles', $fonts_dir = 'fonts', $default_font = array('ayar')){
		...
		$this->browser_name = $browser->getBrowser();
		$this->browser_platform = $browser->getPlatform();
		...
		$this->bulletproof_css_path = $this->file_path_array['basepath'].'/'.$this->file_path_array['bulletproof_path'];
	...
	private function font_format(){
			if ($this->browser_name == 'Internet Explorer'){
				//die("This is $this->browser_platform - $this->browser_name - $this->browser_version");
				if($this->is_mobile && $this->browser_version >= 10){
					$font_format = 'woff';
				}else{
					$font_format = 'eot';
				}
				$extra = '';
			}elseif( ( ( $this->browser_platform == 'Apple' || $this->browser_platform == 'iPhone' || $this->browser_platform == 'iPad' ||$this->browser_platform == 'iPod') && $this->browser_version <= 4.1 ) || ( $this->browser_name == 'Opera' && $this->browser_version <= 9.6 ) || ( $this->browser_name == 'Safari' && $this->browser_version <= 4.1 ) ){
				//die("This is $this->browser_platform - $this->browser_name - $this->browser_version");
				$font_format = 'svg';
			}elseif( $this->browser_platform == 'Android' || ( $this->browser_name == 'Opera' && $this->browser_version < 11.1 ) || ( $this->browser_name == 'Safari' && $this->browser_version < 5.0 ) ){
				//die("This is $this->browser_platform - $this->browser_name - $this->browser_version");
				$font_format = 'ttf';
	...
	private function make_cssfile(){
		...
		$style_dir_path = $this->file_path_array['style_dir_path'];
		//echo $styles_dir;
...
//	$combined_stylesheet = '';
		...
		foreach( $font_family as $k => $font_name ) {
			...
			$stylesheet = $font_name.$format.($is_mobile?'mobile':'').md5($this->server_url).'.css';


			$stylesheet_path = $style_dir_path.'/'.$stylesheet;
...
//			} else {
	...
	public function get_bulletproof(){
		...
		$expires = 60*60*24*14;
		$css_file = $this->bulletproof_css_path;
	...
	public function generate_bulletproof(){
		...
		$css_file_path = $this->bulletproof_css_path;
	...
	private function generate_file_name(){
		...
		$combined_file_name = md5($combined_stylesheet.$browser_name.$url).'.css';
		$combined_file_name_nomd5 = $combined_stylesheet.$browser_name.md5($url).'.css';
		$combined_file_path = $styles_dir.'/'.$combined_file_name;
		$bulletproof_file_name = md5($bp_stylesheet_path.$browser_name.$url).'.css';
		$bulletproof_file_name_nomd5 = $bp_stylesheet_path.$browser_name.md5($url).'.css';
